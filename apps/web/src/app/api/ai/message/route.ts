...
const aiOutputSchema = z
  .object({
    ...
    compliance: z
      .object({
        status: z.enum(["passed", "failed", "flagged", "unchecked"]),
...
async function buildContext(
  ...
  const { data: listings } = await supabase
    .from("listings")
    .select(
      "id, address, price, bedrooms, bathrooms, property_type, status, features",
    )
    .eq("org_id", orgId!)
...
async function callLlm(
  ...
  const ctxStr = JSON.stringify({
    ...
    listings: (context.listings || [])
      .slice(0, 3)
      .map((l: any) => ({ address: l.address, price: l.price })),
  });
  const completion = await requestCopilotCompletion({
    ...
    messages: [
      ...
      {
        role: "user",
        content: "CONTEXT: " + ctxStr + " LEAD MESSAGE: " + userMessage,
      },
    ],
  });
...
const INTENT_AGENT =
  "You are an intent detection specialist for real estate. Analyze the lead message and determine their primary intent. Return JSON: { intent: buying|selling|rental|investment|question|complaint|inspection|negotiation|spam|other, confidence: 0.0-1.0, isRental: boolean }";
...
export async function POST(req: NextRequest) {
  ...
  const requestId = `ai-message-${randomUUID()}`;
  let usageOrgId: string | null = null;
  try {
    ...
    if (!orgId) {
      ...
      if (!internalSecret) {
        console.warn(
          "Internal AI automation disabled: secret is not securely configured",
        );
    ...
    // Keep the whole multi-stage workflow inside one bounded budget. Independent
    // classifiers run concurrently; any failed stage degrades independently.
    const workflowController = new AbortController();
    const workflowTimer = setTimeout(
      () =>
        workflowController.abort(
          new DOMException(
            "AI message workflow deadline exceeded",
            "AbortError",
          ),
        ),
      40_000,
    ...
    const runStage = async <T>(
      ...
      limits: Pick<
        LlmCallOptions,
        "attemptTimeoutMs" | "providerBudgetMs" | "maxAttempts"
      >,
    ): Promise<T> => {
      return runAiMessageStage({
        stage,
        fallback,
        telemetry: stageTelemetry,
        execute: () =>
          callLlm(prompt, message, context, {
            ...limits,
            stage,
        ...
        onFallback: (error) =>
          console.error(
            JSON.stringify({
              level: "error",
              message: "AI message stage degraded",
    ...
    const degradedStages = stageTelemetry
      .filter((event) => event.status === "fallback")
      .map((event) => event.stage);
    const responseDegraded = degradedStages.includes("response");
    const complianceDegraded = degradedStages.includes("compliance");
    ...
    const complianceStatus: "passed" | "failed" | "unchecked" = complianceFailed
      ? "failed"
      : compliancePassed
        ? "passed"
        : "unchecked";
    ...
    const rawOutput = {
      reply:
        responseResult.reply ||
        "Thanks for your message! I will look into that and get back to you.",
      confidence: intent.confidence || 0.5,
      leadStage: stageResult.stage || currentStage,
      nextAction:
        intent.intent === "inspection"
          ? "book_inspection"
          : intent.intent === "negotiation" || intent.intent === "complaint"
            ? "escalate"
            : "reply",
      escalation:
      ...
      escalationReason:
        intent.intent === "negotiation"
          ...
          : intent.intent === "complaint"
            ? "Complaint detected"
            : responseDegraded
              ? "AI response generation unavailable"
              : complianceDegraded
      ...
      scores: {
        buyingReadiness:
          stageResult.stage === "hot"
            ? 0.85
            : stageResult.stage === "warm"
              ? 0.6
              : 0.3,
    ...
    if (leadId) {
      await supabase
        ...
        .update({
          ...
          priority:
            (output.scores?.buyingReadiness || 0) > 0.7
              ? "high"
              : (output.scores?.buyingReadiness || 0) > 0.4
                ? "medium"
                : "low",
        })
        .eq("id", leadId);
      ...
      if (qualification && Object.keys(qualification).length > 0) {
        ...
        if (qualification.preferred_suburbs)
          mem.preferred_suburbs = qualification.preferred_suburbs;
    ...
    const responseTelemetry = stageTelemetry.find(
      (event) => event.stage === "response",
    );
    await recordAIUsage({
      requestId,
      orgId: orgId!,
      userId: user?.id,
      featureKey:
        body.channel === "email" ? "email_automation" : "copilot_chat",
      provider: responseTelemetry?.provider || "local-fallback",
      ...
      metadata: {
        ...
        stages: stageTelemetry.map((event) => ({
          stage: event.stage,
          status: event.status,
    ...
    if (usageOrgId) {
      await recordAIUsage({
        ...
        latencyMs: Date.now() - startTime,
        status: "error",
        errorCode: "ai_message_application_error",
        metadata: { action: "ai_message" },
      });
    }
    return NextResponse.json(
      {
        ...
        reply:
          "Thanks for your message! I have noted it down and someone will get back to you.",
        escalation: true,
