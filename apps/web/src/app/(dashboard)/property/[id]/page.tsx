    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-border bg-card p-4">
          <Bed className="w-5 h-5 text-muted-foreground mb-2" />
          <p className="text-lg font-bold text-foreground">{listing.bedrooms || "-"}</p>
          <p className="text-xs text-muted-foreground">Bedrooms</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <Bath className="w-5 h-5 text-muted-foreground mb-2" />
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <p className="text-lg font-bold text-foreground">{listing.price ? "$" + listing.price : "-"}</p>
          <p className="text-xs text-muted-foreground">Price</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-4">
          <Calendar className="w-5 h-5 text-muted-foreground mb-2" />
          <p className="text-lg font-bold text-foreground">{new Date(listing.created_at).toLocaleDateString()}</p>
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {listing.features.map((f, i) => (
              <span key={i} className="px-3 py-1 bg-muted rounded-full text-xs text-foreground">{f}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        <h2 className="text-lg font-semibold text-foreground mb-3 flex items-center gap-2">
          <Camera className="w-5 h-5 text-muted-foreground" /> Photos
        </h2>
